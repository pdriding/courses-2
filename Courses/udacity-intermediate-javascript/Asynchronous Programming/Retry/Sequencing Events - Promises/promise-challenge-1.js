...
new Promise((resolve, reject) => {
  // your code here
  const unread = alerts.filter((alert) => alert.readStatus);
  if (!unread) throw new Error(`no unread messages`);
  resolve(unread);
})
  .then((test) => {
    const newTest = test.filter((a) => {
      if (a.type === "system") {
        printSystemAlert(a);
        return false; // Exclude 'system' objects
      } else {
  ...
  .then((lastTest) => console.log(lastTest))
