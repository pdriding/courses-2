...
// ***hint = Promise.allSettled

const printAnimalRegions = async (status) => {
  try {
    const animalsList = await animalsByConservationStatus(status);

    const regionsFetch = await animalsList.map(async (animal) => {
      const data = await fetchAnimalByName(animal);

      return data.region;
    });

    const regions = await Promise.allSettled(regionsFetch)
      .then((results) => {
        const regJoin = results.map((result) => result.value);
        return regJoin;
      })
